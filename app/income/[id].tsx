...
import { updateIncome, deleteIncome } from "@/hooks/useIncome";
...
export default function IncomeDetailScreen() {
  ...
  const startEdit = () => {
    if (!item) return;
    setEditAmount((item.income.amount / 100).toString());
    setEditIsYearly(item.income.isYearly);
    setEditCategoryId(item.income.categoryId);
    setEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await updateIncome(item.income.id, {
        amount,
  ...
  const handleDelete = () => {
    Alert.alert(
      "Delete Income",
      "Delete this income item? This cannot be undone.",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await deleteIncome(item!.income.id);
              router.back();
            } catch (e: any) {
  ...
  if (!item) {
    return (
      <View style={styles.centered}>
        <Text style={styles.emptyText}>Income item not found</Text>
      </View>
    );
  }
